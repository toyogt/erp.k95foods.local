...
import { QrCode, CheckCircle2, MapPin, Package, AlertCircle } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { useToast } from '@/components/ui/use-toast';

function ScanField({ label, value, onChange, placeholder, hint }) {
  return (
    <div>
      <Label className="text-xs font-medium text-slate-700 flex items-center gap-1"><QrCode className="w-3.5 h-3.5" />{label}</Label>
      <Input className="h-11 text-base mt-1 font-mono" value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} />
  ...
  async function handleConfirm() {
    ...
    if (existing.length > 0) {
      ...
      await base44.entities.StoreStockBalance.create({
        ...
        uom: resolvedLot.uom,
        quantity: qty,
        mfg_date: resolvedLot.mfg_date,
        expiry_date: resolvedLot.expiry_date,
        putaway_date: now,
        putaway_by: user?.email,
        putaway_id: putawayId,
      });
    ...
    // Update lot status and remaining qty
  ...
  return (
    <div className="space-y-4 max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-5 space-y-4">
        <ScanField label="Step 1 — Scan Location QR" value={locationScan} onChange={setLocationScan} placeholder="Scan or type location code (e.g. WH1-F1-S1-P1-SL1-R2)" hint="Scan the QR code on the rack label" />
        ...
        <ScanField label="Step 2 — Scan Lot QR" value={lotScan} onChange={setLotScan} placeholder="Scan or type Lot ID (e.g. LOT-20241201-0001)" hint="Scan the QR code on the lot label" />
