...
import { Loader2, AlertTriangle, CheckCircle2, Play, Square, ShieldAlert } from 'lucide-react';
import MachineScanner from '@/components/wip/MachineScanner';
import ScanInput from '@/components/wip/ScanInput';
import StartupChecklist from '@/components/labelling/StartupChecklist';
...
export default function LabellingLine() {
  ...
  const [edgeStatus, setEdgeStatus] = useState(null); // null | 'ok' | 'offline'

  const isSupervisor = user?.role === 'admin' || user?.role === 'labelling_supervisor';

  useEffect(() => { base44.auth.me().then(setUser).catch(() => {}); }, []);

  // ── Step 0: Machine confirmed ──────────────────────────────────────────────
  async function handleMachineConfirmed(m) {
    if (!m) return;
    setMachine(m);
    setLoading(true);
    setError('');
    // Determine line from machine_id e.g. "LL-1" → LABEL-LINE-1
    ...
    try {
      ...
      const data = await base44.entities.PackingWO.filter(filter, '-priority', 50);
      setWos(data);
    } catch { setWos([]); }
    setLoading(false);
  ...
  // ── Step 4: Checklist done → create session, call edge ────────────────────
  async function handleChecklistDone(items) {
    setLoading(true);
    ...
    try {
      ...
      await base44.entities.LineChecklistLog.create({
        wo_id: wo.wo_id,
        session_id: sessionId,
        checklist_type: 'STARTUP',
        completed_by: user?.email || '',
        completed_at: now,
        items_json: items,
      });
      await base44.entities.PackingWO.filter({ wo_id: wo.wo_id }).then(async (r) => {
        if (r[0]) await base44.entities.PackingWO.update(r[0].id, { status: 'RUNNING' });
      });
    } catch { /* offline — build local session */ 
    ...
    // Edge: start_job
    const edgeRes = await callEdge('start_job', {
      wo_id: wo.wo_id,
      line_machine_id: machine.machine_id,
      mrp: wo.mrp,
      pack_type: wo.pack_type,
      target_bottles: wo.target_bottles,
      label_sku_code: wo.label_sku_code,
    });
    setEdgeStatus(edgeRes.edge_offline ? 'offline' : 'ok');

    // Edge: printer select message
    if (!edgeRes.edge_offline && wo.printer_template_id) {
      await callEdge('printer_select_message', {
        wo_id: wo.wo_id,
        template_id: wo.printer_template_id,
        variables: wo.print_variables || {},
      });
    }
    ...
    if (newSession?.id) {
      try {
        await base44.entities.LineSession.update(newSession.id, { state: 'RUNNING' });
        newSession.state = 'RUNNING';
      } catch { newSession.state = 'RUNNING'; }
    }
  ...
  // ── Step 5: Crate locked → increment counters ─────────────────────────────
  ...
  // ── Crate error → soft/hard stop ──────────────────────────────────────────
  async function handleCrateError({ reason, crate_id }) {
    if (reason === 'WRONG_ZONE') {
      const newState = session?.state === 'SOFT_STOP' ? 'HARD_STOP' : 'SOFT_STOP';
      const msg = `Crate ${crate_id} from wrong zone — ${newState}`;
  ...
  // ── Supervisor: resume after soft stop ────────────────────────────────────
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── STEP 0: Machine ── */}
      {step === STEP.MACHINE && (
        <MachineScanner stationType="LABEL-LINE" onConfirmed={handleMachineConfirmed} />
      )}
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1">{w.pack_type} · {w.target_bottles} bottles · Priority {w.priority}</p>
                  </div>
      ...
      {step === STEP.SCAN_LABEL && (
        <div className="space-y-3">
          ...
          <ScanInput
            placeholder="Scan label roll barcode…"
            value={labelScan}
            onChange={setLabelScan}
      ...
      {step === STEP.SCAN_CARTON && (
        <div className="space-y-3">
          ...
          <Button variant="ghost" className="w-full" onClick={() => setStep(STEP.CHECKLIST)}>Skip (no carton check)</Button>
        </div>
      )}
      ...
      {step === STEP.CHECKLIST && (
        <div className="space-y-4">
          {loading && <div className="flex justify-center py-6"><Loader2 className="w-6 h-6 animate-spin text-slate-400" /></div>}
          {!loading && <StartupChecklist onComplete={handleChecklistDone} />}
        </div>
      )}
      ...
      {step === STEP.RUNNING && session && (
        <div className="space-y-4">
          ...
          {session.state === 'HARD_STOP' && (
            <div className="bg-red-600 text-white rounded-2xl p-5 flex flex-col items-center gap-2">
              <ShieldAlert className="w-10 h-10" />
              <p className="text-xl font-black">HARD STOP</p>
              <p className="text-sm opacity-90 text-center">{session.reason}</p>
              {isSupervisor && (
                <Button onClick={handleResume} className="mt-2 bg-white text-red-700 hover:bg-red-50 font-bold">
                  Supervisor Override — Resume
                </Button>
              )}
              {!isSupervisor && <p className="text-xs opacity-70">Call supervisor to override</p>}
            </div>
          )}
            ...
            <div className="bg-amber-500 text-white rounded-2xl p-5 flex flex-col items-center gap-2">
              ...
              {isSupervisor && (
                <div className="flex gap-2 mt-2 w-full">
                  <Button onClick={handleResume} className="flex-1 bg-white text-amber-700 hover:bg-amber-50 font-bold">
                    Resume Line
                  </Button>
                  <Button onClick={handleCompleteWO} disabled={loading} className="flex-1 bg-amber-900 hover:bg-amber-800 font-bold text-white">
          ...
          {session.state === 'COMPLETED' && (
            <div className="bg-purple-600 text-white rounded-2xl p-5 flex flex-col items-center gap-3">
              ...
              <Button onClick={() => { setStep(STEP.MACHINE); setMachine(null); setWo(null); setSession(null); setLabelScan(''); setCartonScan(''); }} className="mt-1 bg-white text-purple-700 hover:bg-purple-50 font-bold">
                Start New WO
              </Button>
            </div>
          )}

          {/* Active crate scanner only when RUNNING */}
          ...
          {/* Supervisor complete button when running */}
