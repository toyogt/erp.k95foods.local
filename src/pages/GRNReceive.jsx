...
import { Loader2, RefreshCw, CheckCircle2, Plus, Trash2, Search, AlertTriangle, Camera, Clock, ListChecks, ArrowLeft } from 'lucide-react';
...
export default function GRNReceive() {
  ...
  async function finalizeSubmit(checklistRunId) {
    ...
    setDone({ grn_id, gate_id: selected.gate_id, item_count: validItems.length, supplier_name: uniqueSuppliers.join(', '), received_by: user?.email || '', received_at: new Date().toISOString() });
    setSubmitting(false);
    load();
  }
  ...
  // ── Success ─────────────────────────────────────────────────
  if (done) {
    const doneDate = done.received_at ? new Date(done.received_at).toLocaleDateString('en-IN', { day: '2-digit', month: '2-digit', year: 'numeric' }) : '—';
    return (
      <div className="max-w-4xl mx-auto pt-8 space-y-5 px-2 md:px-0">
        <div className="flex items-center gap-3 bg-green-50 border border-green-200 rounded-xl p-4">
          <CheckCircle2 className="w-8 h-8 text-green-600 shrink-0" />
          <div>
            <h2 className="text-lg font-bold text-green-800">Goods Received Note Confirmed</h2>
            <p className="text-sm text-green-600">{done.item_count} item lot(s) created</p>
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                <tr className="bg-green-50">
                  <td className="px-4 py-3 font-mono font-bold text-slate-900">{done.grn_id}</td>
                  <td className="px-4 py-3 font-mono text-xs text-slate-600">{done.gate_id}</td>
                  <td className="px-4 py-3 text-slate-700">{done.supplier_name || '—'}</td>
                  <td className="px-4 py-3"><span className="px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-700">RECEIVED</span></td>
                  <td className="px-4 py-3 text-xs text-slate-500">{done.received_by || '—'}</td>
                  <td className="px-4 py-3 text-xs text-slate-500">{doneDate}</td>
                </tr>
              </tbody>
            </table>
          </div>
        </div>
        <Button onClick={() => { setSelected(null); setDone(null); setShowCreateForm(false); }} className="w-full h-11 bg-slate-900 text-sm">
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4 pb-12 px-2 md:px-0">
      ...
      {/* Pending Tab */}
      {activeTab === 'pending' && <GRNTable grns={pendingGrns} />}

      {/* Master Tab */}
      {activeTab === 'master' && <GRNTable grns={allGrns} />}

      {/* Create Tab — GRN table + create button */}
      {activeTab === 'create' && !selected && !showCreateForm && (
        <>
          <div className="flex justify-end">
            <Button onClick={() => setShowCreateForm(true)} className="h-11 gap-2 bg-slate-900 text-sm">
              <Plus className="w-4 h-4" /> Create New Goods Received Note
            </Button>
          </div>
          <GRNTable grns={allGrns} />
        </>
      )}

      {/* Create Tab — Gate Entry Selection */}
      {activeTab === 'create' && !selected && showCreateForm && (
        <>
          <div className="flex items-center gap-3">
            <button onClick={() => setShowCreateForm(false)} className="flex items-center gap-1.5 text-sm text-blue-600 font-semibold hover:text-blue-700">
              <ArrowLeft className="w-4 h-4" /> Back to list
            </button>
            <h2 className="text-lg font-bold text-slate-900">Select Gate Entry</h2>
          </div>
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-xs text-slate-400">
                        Arrived: {e.arrived_at ? new Date(e.arrived_at).toLocaleDateString('en-IN', { day: '2-digit', month: '2-digit', year: 'numeric' }) : '—'}
                      </p>
                    </div>
