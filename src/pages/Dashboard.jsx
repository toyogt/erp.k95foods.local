...
import WIPCounters from '@/components/dashboard/WIPCounters';
...
export default function Dashboard() {
  ...
  const [wipCounts, setWipCounts] = useState({ fillingOut: 0, inChamber: 0, inTransit: 0, line1: 0, line2: 0 });
  ...
  async function loadData() {
    try {
      ...
      setStats({
        ...
        pallets: pallets.filter(p => !['CLOSED'].includes(p.status)).length,
        batches: batches.filter(b => ['IN_PROGRESS', 'QC_PENDING'].includes(b.status)).length,
        jobs: jobs.length,
      });

      // WIP location counts
      const countAt = (loc) => activeCrates.filter(c => c.current_location === loc).length;
      const palletCountAt = (status) => pallets.filter(p => p.status === status).length;
      setWipCounts({
        fillingOut: countAt('WIP-FILLING-OUT'),
        inChamber: palletCountAt('IN_CHAMBER'),
        inTransit: palletCountAt('IN_TRANSIT'),
        line1: countAt('ZONE-LABEL-LINE-1'),
        line2: countAt('ZONE-LABEL-LINE-2'),
      });
