      ...
      {tab === 'item' && (
        <div className="space-y-4">
          ...
          {filteredItems.length === 0 ? (
            <div className="bg-white/50 backdrop-blur-xl border border-white/30 rounded-[28px] shadow-[0_4px_24px_rgba(0,0,0,0.06)] p-8 text-center text-slate-400">No stock data matches the filters</div>
          ) : filteredItems.map(item => {
            const visibleEntries = locationFilter ? item.entries.filter(e => e.location_code === locationFilter) : item.entries;
            return (
              <div key={item.item_code} className="bg-white/50 backdrop-blur-xl border border-white/30 rounded-[28px] shadow-[0_4px_24px_rgba(0,0,0,0.06)] overflow-hidden">
      ...
      {tab === 'location' && (
        <div className="space-y-3">
          {Object.entries(byLocation).length === 0 ? (
            <div className="bg-white/50 backdrop-blur-xl border border-white/30 rounded-[28px] shadow-[0_4px_24px_rgba(0,0,0,0.06)] p-8 text-center text-slate-400">No stock data available</div>
          ) : Object.entries(byLocation).map(([locCode, entries]) => (
            <div key={locCode} className="bg-white/50 backdrop-blur-xl border border-white/30 rounded-[28px] shadow-[0_4px_24px_rgba(0,0,0,0.06)] overflow-hidden">
