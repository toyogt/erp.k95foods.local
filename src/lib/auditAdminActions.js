...
export async function auditUserInvited(user, invitedEmail, invitedRole) {
  await createAuditLog(user, 'create', 'USER_MANAGEMENT', 'User', 'invited user', {
...
export async function auditUserRoleChanged(user, targetEmail, fromRole, toRole) {
  await createAuditLog(user, 'update', 'USER_MANAGEMENT', 'User', 'changed user role', {
...
export async function auditRoleCreated(user, roleKey, roleLabel, moduleAccess) {
  await createAuditLog(user, 'create', 'USER_MANAGEMENT', 'AppRole', 'created role', {
    role_key: roleKey,
...
export async function auditRoleUpdated(user, roleKey, roleLabel, moduleAccess) {
  await createAuditLog(user, 'update', 'USER_MANAGEMENT', 'AppRole', 'updated role', {
    role_key: roleKey,
    role_label: roleLabel,
    module_access: moduleAccess,
  });
...
export async function auditRoleDeactivated(user, roleKey, roleLabel) {
  await createAuditLog(user, 'update', 'USER_MANAGEMENT', 'AppRole', 'deactivated role', {
    role_key: roleKey,
    role_label: roleLabel,
  });
}
...
export async function auditApprovalRuleCreated(user, docType, fromState, toState, actionLabel) {
  await createAuditLog(user, 'create', 'ADMIN', 'DocumentApprovalRule', 'created approval rule', {
    doc_type: docType,
...
export async function auditApprovalRuleUpdated(user, docType, fromState, toState) {
  await createAuditLog(user, 'update', 'ADMIN', 'DocumentApprovalRule', 'updated approval rule', {
    doc_type: docType,
    from_state: fromState,
    to_state: toState,
  });
...
export async function auditApprovalRuleDeleted(user, docType, actionLabel) {
  await createAuditLog(user, 'delete', 'ADMIN', 'DocumentApprovalRule', 'deleted approval rule', {
    doc_type: docType,
    action_label: actionLabel,
  });
}
...
export async function auditPermissionPolicyCreated(user, roleKey, moduleAccess) {
  await createAuditLog(user, 'create', 'USER_MANAGEMENT', 'PermissionPolicy', 'created permission policy', {
    role_key: roleKey,
    module_access: moduleAccess,
  });
}
...
export async function auditPermissionPolicyUpdated(user, roleKey, changes) {
  await createAuditLog(user, 'update', 'USER_MANAGEMENT', 'PermissionPolicy', 'updated permission policy', {
    role_key: roleKey,
    changes,
  });
}
...
export async function auditPermissionPolicyDeleted(user, roleKey) {
  await createAuditLog(user, 'delete', 'USER_MANAGEMENT', 'PermissionPolicy', 'deleted permission policy', {
    role_key: roleKey,
  });
}