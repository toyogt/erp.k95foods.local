...
export default function ReceiveQCTab({ user }) {
  ...
  const [currentQc, setCurrentQc] = useState(null); // { serial, checks, photo }
  ...
  async function openSession() {
    ...
    if (!pid) return;
    const links = await base44.entities.BoxPalletLink.filter({ pallet_id: pid }, '-created_date', 500);
  ...
  async function startQC() {
    // load sample_count from AppSetting
  ...
  async function submitQcSample() {
    ...
    // If any FAIL → quarantine pallet
    if (!pass) {
      const pallets = await base44.entities.BoxPallet.filter({ pallet_id: session.pallet_id }, '-created_date', 1);
      if (pallets.length) {
        await base44.entities.BoxPallet.update(pallets[0].id, {
          status: 'QUARANTINE',
    ...
    <div className="space-y-4">
      ...
      {!session && (
        <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-3">
          <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wide">Scan Inbound Pallet</h3>
          <div className="flex gap-2">
            <input ref={palletRef} className="flex-1 h-10 px-3 text-sm rounded-xl border border-slate-300 focus:border-sky-500 focus:outline-none font-mono"
              placeholder="Scan or type Pallet ID…" value={palletInput}
              onChange={e => setPalletInput(e.target.value.toUpperCase())}
              onKeyDown={e => e.key === 'Enter' && openSession()} />
            <Button onClick={openSession} disabled={!palletInput.trim()} className="bg-sky-600 hover:bg-sky-700">Open Session</Button>
          </div>
          <div className="flex items-center gap-2">
            <label className="text-xs font-semibold text-slate-500 uppercase">Warehouse Location:</label>
            <input className="h-9 px-3 text-sm rounded-xl border border-slate-300 focus:outline-none w-48"
        ...
        <>
          <div className="bg-sky-50 border border-sky-200 rounded-2xl px-5 py-3 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-700">Expected: <b>{expectedLinks.length}</b> boxes · Location: <b>{warehouseLocation}</b></p>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-4 space-y-3">
            <div className="flex gap-2">
              <input ref={boxRef} className="flex-1 h-10 px-3 text-sm rounded-xl border border-slate-300 focus:border-sky-500 focus:outline-none font-mono"
                placeholder="Scan Box QR…" value={boxInput} onChange={e => setBoxInput(e.target.value)}
                onKeyDown={e => e.key === 'Enter' && handleBoxScan()} />
              <Button onClick={handleBoxScan} disabled={!boxInput.trim()} className="bg-sky-600 hover:bg-sky-700 h-10">Add</Button>
            </div>
            {scanMsg && (
          ...
          <Button onClick={completeReceiving} disabled={completing || scanned.length === 0}
            className="w-full h-11 bg-slate-900 hover:bg-slate-800 rounded-xl gap-2">
            {completing ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Complete Receiving'}
          </Button>
        </>
      )}
      ...
      {completed && (
        <div className="space-y-3">
          ...
          {qcMode && (
            <div className="bg-white rounded-2xl border border-slate-200 p-5 space-y-4">
              ...
              {!currentQc && qcSamples.length < qcSampleCount && (
                <div className="flex gap-2">
                  <input ref={qcRef} className="flex-1 h-10 px-3 text-sm rounded-xl border border-slate-300 focus:outline-none font-mono"
                    placeholder="Scan QC box…" value={qcBoxInput} onChange={e => setQcBoxInput(e.target.value)}
                ...
                <div className="space-y-3 border border-slate-200 rounded-xl p-4">
                  ...
                  <div>
                    ...
                    <label className="flex items-center gap-2 cursor-pointer text-sm text-sky-600 hover:text-sky-800">
                      ...
                      <input type="file" accept="image/*" className="hidden" onChange={handlePhotoUpload} disabled={qcUploading} />
                    </label>
                  </div>
          ...
          <Button variant="outline" onClick={() => { setSession(null); setPalletInput(''); setCompleted(false); setQcMode(false); }} className="w-full h-10">
            Start New Session
          </Button>
        </div>
      )}
