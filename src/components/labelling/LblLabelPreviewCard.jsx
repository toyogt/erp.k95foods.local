          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-100">
                <th className="text-left px-2 py-1.5 text-slate-600 font-semibold w-1/3">Field</th>
                <th className="text-left px-2 py-1.5 text-slate-600 font-semibold">Computed Value</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              <PodRow pod="POD1" label="MRP" value={fields.mrp ? `₹${fields.mrp}` : '—'} highlight />
              <PodRow pod="POD2" label="MRP with USP" value={fields.mrpWithUsp || '—'} />
              <PodRow pod="POD3" label="Tax Line" value={fields.taxLine} muted />
              <PodRow pod="POD4" label="Batch Number" value={fields.batchNo || '—'} mono />
              <PodRow pod="POD5" label="Manufacturing Date" value={fields.mfgDate || '—'} />
              <PodRow pod="POD6" label="Expiry / Use By Date" value={fields.expiryDate || '—'} warn={!!fields.expiryDate} />
              <PodRow pod="POD7" label="USP" value={fields.usp ? `${fields.usp}` : '—'} />
              <PodRow pod="POD8" label="Manufacturing Date (−1 day)" value={fields.mfgDateOffset || '—'} muted />
              <PodRow pod="POD9" label="Expiry Date (−1 day)" value={fields.expiryDateOffset || '—'} muted />
              <PodRow pod="POD10" label="Net Weight" value={fields.netWeight || '—'} />
              <PodRow pod="POD11" label="USP with Unit" value={fields.uspWithUnit || '—'} />
              <PodRow pod="POD12" label="MRP and USP" value={fields.mrpAndUsp || '—'} />
            </tbody>
          </table>
        </div>
...
function PodRow({ pod, label, value, highlight, warn, muted, mono }) {
  return (
    <tr className="hover:bg-slate-50">
      <td className="px-2 py-1.5 font-mono text-slate-400">{pod}</td>
