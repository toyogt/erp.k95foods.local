    ...
    <div className="relative" ref={containerRef} style={{ zIndex: 99999 }}>
      ...
      {open && (
        ...
        >
          {filteredApproved.length > 0 && (
            <>
              ...
              {filteredApproved.map(s => (
                <div
                  key={s.id}
                  className="px-4 py-2.5 text-sm cursor-pointer hover:bg-slate-50 active:bg-slate-100"
                  onMouseDown={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    handleSelect(s.supplier_name);
                  }}
                >
          ...
          {filteredManual.length > 0 && (
            <>
              ...
              {filteredManual.map(name => (
                <div
                  key={name}
                  className="px-4 py-2.5 text-sm cursor-pointer hover:bg-slate-50 active:bg-slate-100 text-slate-700"
                  onMouseDown={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    handleSelect(name);
          ...
          {showAddNew && (
            <div
              className="px-4 py-2.5 text-sm cursor-pointer hover:bg-blue-50 active:bg-blue-100 flex items-center gap-2 text-blue-600 font-medium border-t border-slate-100"
              onMouseDown={(e) => {
                e.preventDefault();
                e.stopPropagation();
                handleAddNew();
