...
/**
 * Smart supplier input for GRN:
 * - Shows approved suppliers from Supplier entity
 * - Also shows previously-typed manual supplier names (stored in AppSetting)
...
export default function GRNManualSupplierSelect({ value, onChange }) {
  ...
  // Filter manual suppliers (exclude those already shown in approved)
  ...
  // Check if query matches any known entry
  const allKnown = [
  ...
  async function saveManualSupplier(name) {
    ...
    if (manualSettingId) {
      ...
      const rec = await base44.entities.AppSetting.create({
        key: SETTING_KEY,
        value: jsonValue,
        description: 'Manual supplier names used in GRN (store module)',
      }).catch(() => null);
  ...
  function selectApproved(supplier) {
    setQuery(supplier.supplier_name);
    setOpen(false);
    onChange(supplier.supplier_name);
  }
    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute z-[9999] top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-lg max-h-64 overflow-y-auto">
          ...
          {/* Previously-used manual suppliers */}
          {filteredManual.length > 0 && (
            <>
              ...
              {filteredManual.map(name => (
                <div
                  key={name}
                  className="px-4 py-2.5 text-sm cursor-pointer hover:bg-slate-50 text-slate-700"
                  onClick={() => selectManual(name)}
                >
                  {name}
                </div>
              ))}
          ...
          {showAddNew && (
            <div
              className="px-4 py-2.5 text-sm cursor-pointer hover:bg-blue-50 flex items-center gap-2 text-blue-600 font-medium border-t border-slate-100"
