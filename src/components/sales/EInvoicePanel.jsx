 ...
 * Mirrors SI Minimal Workflow:
 *   Pending → Approved & Submitted → Return Initiated → Return Received
 *   → Credit/Debit Note Raised → Send for Tally Posting → Posted to Tally → Delivered
 *
 * Also handles: manual IRN entry, E-Way Bill, and Push to Tally with save
 */
...
import { FileText, Truck, Upload, AlertTriangle, CheckCircle2, Loader2, ArrowRight, RotateCcw } from 'lucide-react';
...
export default function EInvoicePanel({ invoice, order, onUpdated }) {
    ...
    return (
      <div className="flex items-start gap-2 p-4 bg-amber-50 border border-amber-200 rounded-xl">
        ...
        <div>
          ...
          <p className="text-xs text-amber-600 mt-1">Create an invoice from the Invoice tab first, then come here to manage E-Invoice, E-Way Bill, workflow and Tally posting.</p>
        </div>
      </div>
    );
  }
  ...
  async function advanceWorkflow(nextState, extraData = {}) {
    setSaving(true);
    const updateData = { workflow_state: nextState, ...extraData };
    ...
    const eventMap = {
      approved_submitted: 'sales_invoice_approved',
      send_for_tally_posting: 'sales_send_for_tally',
      posted_to_tally: 'sales_tally_posted',
    ...
    if (eventMap[nextState]) {
      ...
      await fireFMSEvent(eventMap[nextState], invoice.id);
    }

    setSaving(false);
    toast({ title: `Invoice → ${INV_WORKFLOW_STEPS.find(s => s.key === nextState)?.label || INV_RETURN_STEPS.find(s => s.key === nextState)?.label || nextState}` });
  ...
  async function saveEInvoiceDetails() {
    ...
    await base44.entities.SalesAuditLog.create({
      ...
      notes: `IRN: ${irn || 'N/A'}, E-Way Bill: ${ewayBill || 'N/A'}`,
    });
    setSaving(false);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4">
        ...
        <div className="flex items-center overflow-x-auto min-w-max gap-0">
          {(isReturnPath ? INV_RETURN_STEPS : INV_WORKFLOW_STEPS).map((step, i) => {
            const idx = isReturnPath ? returnStepIndex : currentStepIndex;
            const done = idx > i;
            const active = idx === i;
            return (
              <div key={step.key} className="flex items-center">
                ...
                {i < (isReturnPath ? INV_RETURN_STEPS : INV_WORKFLOW_STEPS).length - 1 && (
        ...
        <div className="flex items-center justify-between mt-4 flex-wrap gap-2">
          <span className={`text-xs px-3 py-1 rounded-full font-medium ${STATE_COLORS[workflowState] || 'bg-slate-100 text-slate-600'}`}>
            {INV_WORKFLOW_STEPS.find(s => s.key === workflowState)?.label || INV_RETURN_STEPS.find(s => s.key === workflowState)?.label || workflowState}
          </span>
      ...
      {/* Tally Voucher input (show when about to post to Tally) */}
      ...
      {/* Credit/Debit note input (show when raising credit note) */}
        ...
        <div className="bg-purple-50 border border-purple-200 rounded-xl p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div>
              ...
              <Input className="h-9 text-sm mt-1" value={creditNoteNo}
                onChange={e => setCreditNoteNo(e.target.value)} />
            </div>
            <div>
              <Label className="text-xs font-medium text-slate-700">Credit Note Amount</Label>
              <Input type="number" className="h-9 text-sm mt-1" value={creditNoteAmt}
                onChange={e => setCreditNoteAmt(e.target.value)} />
            </div>
          </div>
        </div>
      )}

      {/* Posted to Tally badge */}
      {isPostedToTally && (
        ...
        <div className="p-4 space-y-3">
          {hasEInvoice ? (
            <div className="space-y-2 text-sm">
              ...
              {invoice.ack_number && <div><span className="text-slate-500">Acknowledgement No:</span> <span className="font-medium">{invoice.ack_number}</span></div>}
              {invoice.ack_date && <div><span className="text-slate-500">Acknowledgement Date:</span> <span>{invoice.ack_date}</span></div>}
            </div>
          ) : (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div>
              ...
              <Input className="h-9 text-sm mt-1 font-mono" value={irn} onChange={e => setIrn(e.target.value)} placeholder="Enter IRN" />
            </div>
            <div>
      ...
      {/* Save button for manual E-Invoice/E-Way Bill */}
